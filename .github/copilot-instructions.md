...
- **enigma.gui**: PyQt5 interface ([enigma/gui/main_window.py](enigma/gui/main_window.py)) with chat, logbook, training, and avatar tabs.
...
- **enigma.tools**: OS utilities ([enigma/tools/os_tools.py](enigma/tools/os_tools.py)) and screenshots ([enigma/tools/screenshot.py](enigma/tools/screenshot.py)).

Configuration is centralized in [enigma/__init__.py](enigma/__init__.py) as a CONFIG dict, setting paths for data, models, and DB.

## Developer Workflows
- **Setup**: `python -m venv venv && venv\Scripts\activate && pip install -r requirements.txt` (Windows).
- **Train Model**: `python run.py --train` (uses tiny dataset in data/data.txt; force retrain with `train_model(force=True)`).
...
- **GUI**: Threads for STT to avoid blocking UI ([enigma/gui/main_window.py](enigma/gui/main_window.py#L15-22)).
